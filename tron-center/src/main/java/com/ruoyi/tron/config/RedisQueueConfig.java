...
public class RedisQueueConfig {
	...
	public RedisMessageListenerContainer container(RedisConnectionFactory connectionFactory,
												   ...
												   MessageListenerAdapter listenerAdapterSendMsgPay,
												   MessageListenerAdapter listenerAdapterSendMsgPdai) {
		...
		container.addMessageListener(listenerAdapterSendMsgPay, new PatternTopic("sendMsgPay"));
		container.addMessageListener(listenerAdapterSendMsgPdai, new PatternTopic("sendMsgPdai"));
		return container;
	}
	 ...
	 */
	@Bean
	public MessageListenerAdapter listenerAdapterSendMsgPay(Receiver receiver) {
		MessageListenerAdapter messageListenerAdapter = new MessageListenerAdapter(receiver, "sendMsgPay");
		// 使用Jackson2JsonRedisSerialize 替换默认序列化(默认采用的是JDK序列化)
		Jackson2JsonRedisSerializer<Object> jackson2JsonRedisSerializer = new Jackson2JsonRedisSerializer<>(Object.class);
		ObjectMapper om = new ObjectMapper();
		om.setVisibility(PropertyAccessor.ALL, JsonAutoDetect.Visibility.ANY);
	 ...
	 */
	@Bean
	public MessageListenerAdapter listenerAdapterSendMsgPdai(Receiver receiver) {
		MessageListenerAdapter messageListenerAdapter = new MessageListenerAdapter(receiver, "sendMsgPdai");
		// 使用Jackson2JsonRedisSerialize 替换默认序列化(默认采用的是JDK序列化)
		Jackson2JsonRedisSerializer<Object> jackson2JsonRedisSerializer = new Jackson2JsonRedisSerializer<>(Object.class);
		ObjectMapper om = new ObjectMapper();
		om.setVisibility(PropertyAccessor.ALL, JsonAutoDetect.Visibility.ANY);
